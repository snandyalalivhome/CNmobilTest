...
define('PUSHSECRET','x-GupjLOQx23h3SDDvmzSA'); 
...
class GalleryHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
       ...
       // $config = parse_ini_file((dirname(__DIR__) . '../../config.ini'));  
          ...
          if(isset($_POST["UserID"]))
        {
         ...
         $appPath = str_replace('__USERID__', $currentUserId, $savein);
    ...
    protected function get_download_url($file_name, $version = null, $direct = false) {
        ...
        $tmp=$this->options['upload_url'].$this->get_user_path().$version_path.rawurlencode($file_name);
    ...
    protected function set_additional_file_properties($file) {
        ...
        // set by snandyala to delete by entity Id
        $file->frameID= $this->getFrameID($file->name);
    }

    protected function getFrameID($file_name)
    {
           $getGallery= new assetframeshareModel();
          $framecollection= $getGallery->getEntityId($file_name);
          if($framecollection!=null)
          {
          return $framecollection['entity_id'];
          }
          else
              return '';
    }
    ...
    protected function is_valid_file_object($file_name) {
        //do not display image versions - snandyala
         $versionfiles= strpos($file_name, "_");
         if($versionfiles !=0)
         {
             return false;
         }
         ...
         if(isset($_REQUEST["UserID"]))
        {
         ...
         $getGallery= new assetframeshareModel();
          $framecollection= $getGallery->getFrame($currentUserId);
        
          if(count($framecollection )!=0)
          {
              $exists=false;
              for($i=0;$i<count($framecollection);$i++)
              { 
                  $frame= $framecollection[$i];
                if(in_array($file_name, $frame))
                {
                    $exists= true;
                    break;
                }
    ...
    protected function get_file_objects($iteration_method = 'get_file_object') {
        ...
        {
            $thisimg= $imgs[$i];
             $imgname= $thisimg->name;
             
            $versionfiles= strpos($imgname, "_");
           if($versionfiles === true)
          {
               return null;
          }
    ...
    public function post($print_response = true) {
        ...
        //custom code snandyala
	   if(isset($_REQUEST["button"]))
			{
			
			$myboxes=$_REQUEST['button']; 
			$data = json_decode($myboxes);

				// $myboxes = $_POST["button"];
					 if(!empty($data))
					  {
						   ...
						   for($j=0; $j < count($data); $j++)
							...
							return $this->custom_delete($print_response, $data);
    ...
    public function delete($print_response = true) {
     ...
     public function custom_delete($print_response = true, $filesToDelete) {
        ...
        $response = array();
        foreach($filesToDelete as $entity_id){
            
            $deleteModal=new assetframeshareModel();
            $assetdeleted= $deleteModal->deleteFrameShare($entity_id);
            if($assetdeleted)
            {
